...
export function HomeContinueWatching() {
  ...
  const resume = (entry: typeof entries[number]) => {
    ...
    router.push({
      pathname: '/player/[id]',
      params: {
        id: String(mediaIdentity.id), type: mediaIdentity.mediaType,
    ...
    <View style={styles.section}>
      <View style={[styles.header, { paddingHorizontal: padding }]}>
        <View style={styles.headingCopy}>
          <Text style={[styles.eyebrow, { color: theme.accent }]}>PICK UP THE STORY</Text>
          <Text style={[styles.title, { color: theme.text }]}>Continue Watching</Text>
        </View>
        <Pressable
          accessibilityRole="button"
          accessibilityLabel="View all Continue Watching"
          onPress={() => router.push({ pathname: '/(tabs)/library', params: { tab: 'continue' } })}
          style={({ pressed }) => [styles.viewAll, { borderColor: theme.border }, pressed && styles.pressed]}
        >
...
const styles = StyleSheet.create({
  ...
  header: { minHeight: 72, paddingTop: 12, paddingBottom: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12 },
  headingCopy: { flex: 1, minWidth: 0 },
  eyebrow: { fontSize: 10, fontWeight: '900', letterSpacing: 1.7, marginBottom: 3 },
  title: { fontSize: 22, lineHeight: 28, fontWeight: '900', letterSpacing: -0.5 },
  viewAll: { minHeight: 44, borderWidth: 1, borderRadius: 22, paddingHorizontal: 13, flexDirection: 'row', alignItems: 'center', gap: 3 },
