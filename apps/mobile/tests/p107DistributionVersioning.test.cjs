...
test('P10.7-A production candidate owns one aligned Mobile version identity', () => {
  assert.equal(app.expo.version, '2.2.14');
  assert.equal(app.expo.android.versionCode, 48);
  assert.equal(app.expo.android.package, 'com.okali.orion');
  assert.equal(mobilePackage.version, '2.2.14');
  assert.equal(rootLock.packages['apps/mobile'].version, '2.2.14');
});
