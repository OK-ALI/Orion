...
function harness({ mode = 'home', state = 'offline', presentation, width = 480, height = 980, fontScale = 1,
  ...
  const mocks = {
    ...
    'react-native': { ...Object.fromEntries(['View', 'Text', 'ScrollView', 'FlatList', 'ActivityIndicator', 'Pressable', 'Image', 'Modal'].map((s) => [s, s])),
      StyleSheet: { create: (s) => s, absoluteFill: {} }, Platform: { OS: 'android' }, useWindowDimensions: () => ({ width, height, fontScale }) },
    ...
    'expo-file-system': {
      Directory: class Directory { constructor() { this.uri = 'file:///artwork/'; this.exists = false; } create() { this.exists = true; } list() { return []; } },
      File: class File { constructor(...parts) { this.uri = String(parts.at(-1) || 'file:///artwork/file'); this.exists = false; this.size = 0; } delete() { this.exists = false; } },
