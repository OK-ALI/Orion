  ...
  useEffect(() => {
    const handler = (event) => {
      ...
      if (isLocal && nativeVideoRef.current) {
        ...
        if (command === "seekBackward") video.currentTime = Math.max(0, video.currentTime - 10);
        if (command === "seekForward") video.currentTime = Math.min(video.duration || Infinity, video.currentTime + 10);
        if (command === "toggleMute") video.muted = !video.muted;
        if (command === "volumeUp") { video.muted = false; video.volume = Math.min(1, video.volume + 0.05); }
        if (command === "volumeDown") video.volume = Math.max(0, video.volume - 0.05);
        if (String(command).startsWith("seek:")) {
          const seconds = Number(String(command).slice(5));
          if (Number.isFinite(seconds)) video.currentTime = Math.max(0, seconds);
        }
        if (String(command).startsWith("speed:")) {
          const rate = Number(String(command).slice(6));
