...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: "connect", label: "Orion Connect", icon: ConstellationIcon },
    ],
  },
];
...
export default function Sidebar({
  ...
  useEffect(() => {
    // Check initial smart connect status
  ...
  useEffect(() => {
    const toggleFromRemote = () => {
      if (collapsed) revealSidebar();
      else requestMode(SIDEBAR_MODES.COLLAPSED);
    };
    window.addEventListener("orion:toggle-sidebar", toggleFromRemote);
    return () => window.removeEventListener("orion:toggle-sidebar", toggleFromRemote);
  }, [collapsed, requestMode, revealSidebar]);
                ...
                {id === "connect" && (
                  <span
                    style={{
                      ...
                      height: 8,
                      borderRadius: 4,
                      background: isConnectLive ? "#10b981" : "#f59e0b",
                      boxShadow: isConnectLive ? "0 0 10px rgba(16, 185, 129, 0.9)" : "none",
                      marginLeft: "auto",
