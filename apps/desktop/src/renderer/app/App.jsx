...
export default function App() {
  ...
  useEffect(() => {
    ...
    const handleRemoteCommand = async (payload) => {
      ...
      // Direct Navigation & Sidebar Focus Commands
      const SIDEBAR_PAGES = ["home", "search", "discover", "constellation", "library", "downloads", "music-home", "settings"];

      // ── Virtual Touchpad Laser Pointer Handling ──
      if (action === "cursor_move") {
        ...
        const xRatio = Math.max(0, Math.min(1, Number(pointer.x ?? pointer.xRatio) || 0));
        const yRatio = Math.max(0, Math.min(1, Number(pointer.y ?? pointer.yRatio) || 0));
        ...
        if (elemBelow) {
          const interactive = elemBelow.closest(".media-card, button, a, [role='button'], [tabindex='0']");
          document.querySelectorAll(".spatial-remote-focused").forEach((el) => el.classList.remove("spatial-remote-focused"));
          if (interactive) {
      ...
      if (action === "cursor_click") {
        ...
        if (cursor) {
          ...
          if (elemBelow) {
            const clickable = elemBelow.closest(".media-card, button, a, [role='button'], input") || elemBelow;
      ...
      if (action === "navigate_page") {
        if (value) baseNavigate(value);
      }
      if (action === "sidebar_next") {
        const curIdx = SIDEBAR_PAGES.indexOf(pageRef.current || "home");
        const nextIdx = (curIdx + 1) % SIDEBAR_PAGES.length;
        baseNavigate(SIDEBAR_PAGES[nextIdx]);
      }
      if (action === "sidebar_prev") {
        const curIdx = SIDEBAR_PAGES.indexOf(pageRef.current || "home");
        const prevIdx = (curIdx - 1 + SIDEBAR_PAGES.length) % SIDEBAR_PAGES.length;
        baseNavigate(SIDEBAR_PAGES[prevIdx]);
      }
      // ── Spatial Focus Navigation Engine for Media Cards & Posters ──
      if (action === "focus_card_next" || action === "focus_card_prev") {
        const selector = ".media-card, [role='button'], .poster-container, .card, .search-media-result";
        ...
        if (cards.length > 0) {
          ...
          const currentFocused = document.querySelector(".spatial-remote-focused");
          let curIdx = currentFocused ? cards.indexOf(currentFocused) : -1;
          if (curIdx === -1) {
            const active = document.activeElement;
            curIdx = cards.indexOf(active);
          }
          
          let nextIdx;
          if (action === "focus_card_next") {
            nextIdx = curIdx === -1 ? 0 : (curIdx + 1) % cards.length;
          } else {
            nextIdx = curIdx === -1 ? cards.length - 1 : (curIdx - 1 + cards.length) % cards.length;
          }

          document.querySelectorAll(".spatial-remote-focused").forEach((el) => el.classList.remove("spatial-remote-focused"));
          
          const targetEl = cards[nextIdx];
          if (targetEl) {
            targetEl.classList.add("spatial-remote-focused");
      ...
      if (action === "play_media") {
        ...
        if (targetId && typeof baseNavigate === "function") {
          baseNavigate(targetType, targetId);
        }
      }

      if (action === "constellation_search") {
        baseNavigate("constellation");
        window.dispatchEvent(new CustomEvent("orion:constellation-search", { detail: value }));
      }

      // D-Pad Navigation & Scroll
      if (action === "up") {
        if (typeof targetScroll.scrollBy === "function") {
          targetScroll.scrollBy({ top: -280, behavior: "smooth" });
        } else {
      ...
      if (action === "left") {
        ...
        if (active && active !== document.body) {
          ...
          targetScroll.scrollBy({ left: -240, behavior: "smooth" });
        }
      }
      if (action === "right") {
        const active = document.activeElement;
        if (active && active !== document.body) {
          active.dispatchEvent(new window.KeyboardEvent("keydown", { key: "ArrowRight", bubbles: true }));
        } else {
          targetScroll.scrollBy({ left: 240, behavior: "smooth" });
        }
      }
      if (action === "select") {
        const remoteFocused = document.querySelector(".spatial-remote-focused");
        if (remoteFocused && typeof remoteFocused.click === "function") {
          remoteFocused.click();
        } else {
      ...
      if (action === "send_text") {
        setShowSearch(true);
        if (value && typeof baseNavigate === "function") {
          baseNavigate("search", value);
        }
      }

      // HUD Playback & Media Controls
      if (action === "toggle_play" || action === "play_pause") {
        commandResult = await handleSystemMediaCommand("toggle");
      }
      if (action === "seek_-10") {
        commandResult = await handleSystemMediaCommand("seekBackward");
      }
      if (action === "seek_+10") {
        commandResult = await handleSystemMediaCommand("seekForward");
      }
      if (action === "previous") {
        commandResult = await handleSystemMediaCommand("previous");
      }
      if (action === "next") {
        commandResult = await handleSystemMediaCommand("next");
      }
      if (action === "toggle_mute") {
        commandResult = await handleSystemMediaCommand("toggleMute");
      }
      if (action === "volume_up") {
        commandResult = await handleSystemMediaCommand("volumeUp");
      }
      if (action === "volume_down") {
        commandResult = await handleSystemMediaCommand("volumeDown");
      }
      if (action === "set_speed") {
        commandResult = await handleSystemMediaCommand(`speed:${Number(value)}`);
      }
      if (action === "toggle_subtitles") {
        commandResult = await handleSystemMediaCommand("toggleSubtitles");
      }
      if (action === "toggle_fullscreen") {
        commandResult = await window.electron?.toggleFullscreen?.()
          || { ok: false, error: "Desktop fullscreen control is unavailable." };
      }
      if (action === "toggle_pip") {
        createMiniHandoff();
      }

      if (payload?.id && window.electron?.acknowledgeSmartConnectCommand) {
        window.electron.acknowledgeSmartConnectCommand({
          id: payload.id,
          ...
          pointer: action === "cursor_move"
            ? {
                x: Math.max(0, Math.min(1, Number(payload?.pointer?.x ?? payload?.value?.x ?? payload?.value?.xRatio) || 0)),
                y: Math.max(0, Math.min(1, Number(payload?.pointer?.y ?? payload?.value?.y ?? payload?.value?.yRatio) || 0)),
              }
            : undefined,
        }).catch(() => {});
      }
    ...
    if (window.electron?.onRemoteCommand) {
      unsubscribeElectron = window.electron.onRemoteCommand(handleRemoteCommand);
    }

    // BroadcastChannel sync across web tabs
    let bc;
    try {
      if (typeof window.BroadcastChannel !== "undefined") {
        bc = new window.BroadcastChannel("orion_smart_connect");
        bc.onmessage = (event) => {
    ...
    const handleCustomRemote = (e) => handleRemoteCommand(e.detail);
    window.addEventListener("orion:remote-command-custom", handleCustomRemote);

    return () => {
      if (unsubscribeElectron) unsubscribeElectron();
      try { bc?.close(); } catch (e) {}
      window.removeEventListener("orion:remote-command-custom", handleCustomRemote);
    };
