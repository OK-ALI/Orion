...
export default function ConstellationPage({ apiKey, history = [], saved = [], offline = false, onNavigate }) {
  ...
  useEffect(() => {
    const handleConstellationSearch = (e) => {
      if (typeof e.detail === "string") {
        setQuery(e.detail);
      }
    };
    window.addEventListener("orion:constellation-search", handleConstellationSearch);
    return () => window.removeEventListener("orion:constellation-search", handleConstellationSearch);
  }, []);
