...
const isRemoteUnavailable = (event) => !["online", "degraded"].includes(connectionStates.get(event.sender));
...
function register() {
  ...
  ipcMain.handle(MUSIC_IPC.CONNECTION_SET, (event, state) => {
    if (!["online", "offline", "degraded", "unknown", "checking", "reconnecting"].includes(state)) return { ok: false };
  ...
  ipcMain.handle(MUSIC_IPC.TRACK_GET_STREAM, async (event, track, providerId) => {
    if (isRemoteUnavailable(event) && track?.provider !== "local") return connectionRequired();
  ...
  ipcMain.handle(MUSIC_IPC.TRACK_CANDIDATES, async (event, track, providerId) => {
    if (isRemoteUnavailable(event) && track?.provider !== "local") return { ...connectionRequired(), candidates: [] };
  ...
  ipcMain.handle(MUSIC_IPC.TRACK_RESOLVE_CANDIDATE, async (event, id) => {
    try { return { ok: true, ...(await resolver.resolveCandidateGrant(id, { localOnly: isRemoteUnavailable(event) })) }; }
  ...
  ipcMain.handle(MUSIC_IPC.ARTWORK_GET, async (event, track = {}) => {
    ...
    if (artworkUrl) {
      try {
        const filePath = await cacheRemoteArtwork(artworkUrl, { localOnly: isRemoteUnavailable(event) });
        return { ok: true, ...tokens.createGrant({ kind: "artwork", filePath, mimeType: "image/png" }) };
      } catch {}
    }
    if (isRemoteUnavailable(event)) return connectionRequired();
  ...
  ipcMain.handle(MUSIC_IPC.LYRICS_GET, async (event, track) => {
    ...
    for (const provider of providers) {
      ...
      if (isRemoteUnavailable(event) && provider.id !== "orion-embedded-lyrics") continue;
      try {
  ...
  ipcMain.handle(MUSIC_IPC.SEARCH, async (event, query) => {
    ...
    if (isRemoteUnavailable(event)) return localSearch();
  ...
  ipcMain.handle(MUSIC_IPC.SEARCH_SUGGESTIONS, async (event, query) => {
    if (isRemoteUnavailable(event)) return { ...connectionRequired(), suggestions: [] };
  ...
  ipcMain.handle(MUSIC_IPC.RADIO_GET, async (event, item = {}) => {
    if (isRemoteUnavailable(event)) return { ...connectionRequired(), tracks: [] };
  ...
  ipcMain.handle(MUSIC_IPC.DETAILS_GET, async (event, kind, item = {}) => {
    ...
    if (isRemoteUnavailable(event) && !localDetail) return connectionRequired();
  ...
  ipcMain.handle(MUSIC_IPC.PLAYLISTS_REMOTE_LIST, async (event) => {
    if (isRemoteUnavailable(event)) return { ...connectionRequired(), sources: [] };
