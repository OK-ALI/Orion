function createRealtimeDiagnostics() {
  ...
  const timer = enabled ? setInterval(() => {
    console.log(
      `[SmartConnect realtime] received=${counters.received} rateRejected=${counters.rateRejected} replayRejected=${counters.replayRejected} coalesced=${counters.coalesced} forwarded=${counters.forwarded}`,
    );
    counters.received = 0;
