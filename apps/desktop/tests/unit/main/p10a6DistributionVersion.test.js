...
test("Orion v3.0 production release owns aligned Desktop and Mobile 3.0.0 identities", () => {
  ...
  assert.equal(desktopPackage.version, "3.0.0");
  assert.equal(packageLock.packages["apps/desktop"].version, "3.0.0");

  assert.equal(rootPackage.version, "3.0.0");
  ...
  assert.equal(packageLock.packages["apps/mobile"].version, "3.0.0");
  assert.equal(mobileApp.expo.version, "3.0.0");
  assert.equal(mobileApp.expo.android.versionCode, 55);
});
