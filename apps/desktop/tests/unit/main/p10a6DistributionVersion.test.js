...
test("P11.3 production candidate owns aligned Desktop and Mobile 2.2.21 identities", () => {
  ...
  assert.equal(desktopPackage.version, "2.2.21");
  assert.equal(packageLock.packages["apps/desktop"].version, "2.2.21");
  ...
  assert.equal(mobilePackage.version, "2.2.21");
  assert.equal(packageLock.packages["apps/mobile"].version, "2.2.21");
  assert.equal(mobileApp.expo.version, "2.2.21");
  assert.equal(mobileApp.expo.android.versionCode, 54);
});
