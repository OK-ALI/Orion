...
test("Orion v3.0.1 hotfix release owns aligned Desktop and Mobile 3.0.1 identities", () => {
  ...
  assert.equal(desktopPackage.version, "3.0.1");
  assert.equal(packageLock.packages["apps/desktop"].version, "3.0.1");

  assert.equal(rootPackage.version, "3.0.1");
  ...
  assert.equal(packageLock.packages["apps/mobile"].version, "3.0.1");
  assert.equal(mobileApp.expo.version, "3.0.1");
