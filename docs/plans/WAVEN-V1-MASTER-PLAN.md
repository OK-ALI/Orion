...
- **Provider-backed discovery:** Home calls the existing `wavenDiscoveryRuntime.getDashboard()` path with `AbortController` cancellation. Provider-curated dashboard entities are projected into bounded **Songs**, **Artists**, **Albums**, and **Playlists** groups inside Explore when those entity kinds are actually present. Exact provider section titles and attribution are intentionally not rendered on the WAVEN product surface.
