...
- **P6.3 stable-Home + semantic-icon correction:** physical comparison with and without a VPN proved that anonymous `FEmusic_home` shelf availability can change enough to add/remove a core WAVEN Home lane. That provider variability may influence which items are returned, but it must not own WAVEN's page composition. P6.3 therefore keeps dashboard/catalog shelves when present, fills only missing Songs/Artists/Albums/Playlists lanes through type-directed public discovery, and removes the generic `Top songs` whole-Home fallback. Explore category tiles also stop using `WavenArtworkFallback`; navigation destinations use dedicated WAVEN semantic icons, while artwork and deterministic fallback artwork are reserved for real music entities.
