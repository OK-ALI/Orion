...
- Keep the existing Songs / Artists / Albums Explore shortcuts and append real provider-backed discovery beneath them. Explore destinations use purpose-built WAVEN semantic icons, not fallback media artwork: **Icons represent destinations and actions; artwork represents music entities.** The Songs icon uses explicit geometric note parts rather than a font glyph, and its WAVEN-blue accent is integrated into the note itself. The Artists icon uses a clean creator silhouette plus integrated signal bars rather than a microphone, account badge, detached dot, or masked shape that can render like a broken profile icon.
...
- Artist shelves must preserve provider semantics: only entities explicitly marked `MUSIC_PAGE_TYPE_ARTIST` may render as WAVEN Artists. A generic `UC*` channel browse identity by itself is not enough to classify an entity as an artist. For a missing Home Artists lane, use the evidence-backed `top music artists` source first and `top artists` as a bounded secondary source, deduplicating by stable entity identity rather than weakening Artist typing.
- Home discovery must preserve query intent. The generic `Top songs` whole-Home fallback is retired. WAVEN owns a stable four-type discovery foundation: real dashboard/catalog entities are preferred, and any missing track / artist / album / playlist lane is backfilled only by category-directed public discovery whose results are filtered through the existing strict entity classifier. These entity lanes are an internal discovery foundation, not permanent product-facing database labels. In the current truthful non-personalized P6.3 state, the visible Home shelves are **Popular Now**, **Popular Artists**, **Popular Albums**, and **Popular Playlists**. This keeps the accepted Reference-02 editorial rhythm without fabricating personalization, history, or chart ranking. A future **Trending Now** title requires a real trend/chart signal rather than merely renaming a generic discovery query. Exact catalog items may still vary with provider availability, but a variable provider response must not by itself add or remove a core Home lane. The attempted VPN comparison in Expo Go is not accepted as region evidence because the VPN did not route Expo Go traffic; repeated Home re-entry instead demonstrated ordinary anonymous-provider result variability.
- Track normalization must reject explicit non-music rows before `videoId` can promote them to WAVEN Songs. `Episode`, `Podcast`, and `MUSIC_PAGE_TYPE_NON_MUSIC_AUDIO_TRACK_PAGE` semantics are not Songs; legitimate provider music-video rows remain eligible unless another explicit non-music semantic excludes them.
...
- **P6.3 Home visual-hierarchy refinement:** the physically reviewed Home must use one editorial card rhythm across real provider-backed shelves, derived from Reference 02 rather than mixing multiple media-card grammars. Popular Now / Artists / Albums / Playlists share the actual Reference-02 card composition translated into WAVEN: substantial rounded artwork remains visually dominant inside a restrained near-black glass card, title and one-line secondary metadata remain inside the same vertically coherent card body, cards keep consistent height/baselines, and the horizontal rail uses tighter spacing plus a visible continuation/peek rather than reading as a fixed three-column grid. This is not a blanket shrink pass; the artwork stays substantial while the card shell and rail become denser and more editorial. Major Home headings use WAVEN identity by keeping the leading word white and the semantic word in WAVEN blue (for example **Recently _Played_**, **Popular _Now_**, **Popular _Artists_**, **Popular _Albums_**, **Popular _Playlists_**, and **Your _Music_**). The single-word Explore heading remains white with a restrained micro-wave/signal mark rather than a detached dot. Explore remains navigation and therefore keeps semantic icons rather than artwork; Songs uses a clean paired-note geometry with the blue accent built into the second note, Artists uses a clean creator silhouette with integrated blue signal bars rather than a microphone/profile-only glyph, and Albums retains the album/disc identity. The icon field must not add another square tile inside the already-rounded Explore card; one restrained circular halo is enough. Explore cards and the truthful empty Recently Played surface inherit the same glass/radius/spacing family without pretending navigation or empty state is real media. No play-button overlay may be introduced before the owning playback/detail interaction exists.
- **P6.3 physical-integrity follow-up:** the first chart/cache/card repair physically proved that `Popular Now` can render the current chart feed and that Home → Search/Library → Back preserves the same Home snapshot. The follow-up therefore keeps the chart and session-cache architecture, but closes the remaining product-quality gaps: an explicit refresh is one-shot rather than making every later Home mount bypass the session cache; a degraded-ready Home exposes a real 48dp Refresh action; media cards use one explicit centered content column whose width equals the artwork width, so artwork, title, and metadata share the same left/right edges inside symmetric card gutters rather than drifting toward the leading rounded edge; and Explore removes nested icon chrome, font-dependent music glyphs, detached blue dots, and the artist mask artifact.
- **P6.3 selective WAVEN Glass refinement:** Home may deepen the already-locked WAVEN Glass material without adding a blur dependency. Real media artwork remains crisp and visually dominant; the media-card shell receives only a very light translucent blue tint, quiet cool-silver/blue inner edge, and slightly clearer top edge. Utility/navigation surfaces such as Explore, Recently Played empty state, Home discovery states, and Your Music may use the stronger of the two Home glass tiers because they are UI chrome rather than music artwork. The hero remains unchanged. No `BlurView`, `expo-blur`, backdrop filter, dependency upgrade, provider behavior, playback behavior, persistence, Cloud, or later-phase scope is introduced.
...
### P6.3 Home integrity correction — chart truth, session continuity, and card polish

The physical Reference-02 review established four additional product rules:

- `Popular Now` is a chart claim and therefore requires a real ranked current-popularity signal. Generic searches such as `popular songs` are discovery relevance, not evidence of popularity.
- WAVEN uses the existing public global Top-50 chart signal as the primary track shelf. If that chart is unavailable, the same slot must become **Discover Now** rather than falsely retaining **Popular Now**.
- Artist, album, and playlist lanes sourced from provider discovery are labeled **Featured Artists**, **Featured Albums**, and **Featured Playlists** until a ranked popularity source explicitly owns those claims.
- Home discovery is stable for the current app session. Navigating Home → Search/Library → Back restores the same discovery snapshot. Explicit retry may replace that snapshot. This is Phase-6 ephemeral discovery caching, not Phase-7 Library/history persistence.
- Reference-02 media cards keep substantial artwork but use a wider/taller editorial footprint, quieter glass edges, and a padded metadata column so titles and type/artist text never collide visually with rounded card edges.
- Explore icons must render as simple, optically centered identity marks. Songs uses a reliable music-note glyph with a restrained WAVEN identity accent; Artists uses a clean bust silhouette; Albums retains the disc/cover identity.
...
### P6.3 Home / Discovery physical acceptance lock — 2026-09-17

- **Status:** P6.3 Home / Discovery is **ACCEPTED / LOCKED** at this evidence boundary.
- The physical Expo Go review accepts the final Home composition and material language: near-black WAVEN base, cyan/blue/navy/cool-silver atmosphere, restrained selective glass, crisp real artwork, centered Reference-02 media cards, tighter vertical rhythm, intentional third-card continuation peek, simple Songs note, accepted Artists signal icon, and accepted Albums identity icon.
- Product semantics remain truthful: **Popular Now** is chart-backed when the ranked current chart is available and becomes **Discover Now** otherwise; provider-discovery shelves remain **Featured Artists**, **Featured Albums**, and **Featured Playlists**.
- The previously verified Home → Search/Library → Back behavior remains the accepted physical evidence that ordinary navigation restores the same one-session Home discovery snapshot. Explicit Refresh is the only intended request-level cache bypass; the accepted atmospheric micro-polish does not alter that behavior.
- The accepted automated boundary is TypeScript PASS plus **141/141** WAVEN tests. This record adds no dependency, provider, playback, persistence, Orion Cloud, offline, or Phase 7+ ownership.
- Expo Go closes only the P6.3 fetch/UI physical-evidence tier. Native playback and signed distribution remain owned by their later validation tiers.
- Freeze this Home visual language for subsequent Phase 6 work unless a later defect is proven. Do not reopen broad visual exploration while implementing Artist/Album detail, pagination, caching/health, attribution, or search-to-play handoff.
