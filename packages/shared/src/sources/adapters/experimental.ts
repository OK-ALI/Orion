/**
 * Experimental & Disabled Cinema Sources
 *
 * Ported from desktop's adapters/experimental.js
 */

import type { CinemaSourceDescriptor } from "../contracts";

const media = Object.freeze({ movie: true, tv: true, anime: false });
const tmdb = Object.freeze({ movie: "tmdb" as const, tv: "tmdb" as const });

export const experimentalSources: CinemaSourceDescriptor[] = [
  {
...
export const disabledSources: CinemaSourceDescriptor[] = (
  [
  ...
  id,
  label,
  releaseStatus: "disabled" as const,
  media,
  idPolicy: tmdb,
  buildMovieUrl: (mediaId: string | number) => `${origin}/embed/movie/${mediaId}`,
  buildEpisodeUrl: (mediaId: string | number, season: number, episode: number) => `${origin}/embed/tv/${mediaId}/${season}/${episode}`,
