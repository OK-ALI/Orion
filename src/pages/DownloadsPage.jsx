...
export default function DownloadsPage({
  ...
  const active = useMemo(
    () => downloads.filter((d) => d.status === "downloading" || d.status === "interrupted"),
    [downloads],
  );
  ...
  const finished = useMemo(
    () =>
      downloads
        .filter((d) => d.status !== "downloading" && d.status !== "interrupted")
  ...
  const handlePause = useCallback(async (dl) => {
    if (!isElectron) return;
    const res = await window.electron.pauseDownload(dl.id);
    if (res.ok) {
      onUpdateDownload?.(dl.id, { status: "interrupted", lastMessage: "Paused" });
    }
  }, [onUpdateDownload]);

  const handleResume = useCallback(async (dl) => {
    if (!isElectron) return;
    const check = window.electron.getDownloaderStatus
      ? await window.electron.getDownloaderStatus()
      : await window.electron.checkDownloader();
    if (!check.exists) {
      alert("yt-dlp and ffmpeg are not ready. Install downloader tools from a download dialog or Settings, then retry.");
      return;
    }
    const res = await window.electron.resumeDownload({ id: dl.id });
    if (res.ok) {
      onUpdateDownload?.(dl.id, { status: "downloading", lastMessage: "Resuming…" });
    } else {
      alert(`Failed to resume: ${res.error}`);
    }
  }, [onUpdateDownload]);
          ...
          <div className="dl-page__list">
            {active.map((dl) => (
              <ActiveCard
                ...
                onPause={() => handlePause(dl)}
                onResume={() => handleResume(dl)}
                onSelect={
...
function PauseIcon({ size = 20, ...props }) {
  return (
    <svg
      width={size}
      height={size}
...
const ActiveCard = memo(function ActiveCard({ dl, onDelete, onSelect, onPause, onResume }) {
