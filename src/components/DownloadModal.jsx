...
export default function DownloadModal({
  ...
  const [subEnabled, setSubEnabled] = useState(
    () => storage.get(STORAGE_KEYS.SUBTITLE_ENABLED) === 1,
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const result = window.electron.getDownloaderStatus
          ? await window.electron.getDownloaderStatus()
          : await window.electron.checkDownloader(downloaderFolder);
        if (!mounted) return;
  ...
  useEffect(() => {
    if (!isElectron || !window.electron?.onDownloaderToolsProgress) return;
  ...
  const installTools = async () => {
    if (!isElectron || !window.electron?.installDownloaderTools) return;
    setInstallingTools(true);
    setInstallProgress({ step: "Starting", progress: 0 });
    const result = await window.electron.installDownloaderTools();
    if (result.ok) {
      setDownloader(result.status);
      setInstallProgress({ step: "Ready", progress: 100 });
    } else {
      setDownloadStatus(result.error || "Could not install downloader tools");
  ...
  const handleDownload = async () => {
    if (!downloader?.exists || !downloadPath || !m3u8Url) return;
            ...
            <>
              ...
              >
                ...
                >
                  ...
                  <span style={{ fontSize: 12, color: "var(--text3)" }}>
                    Optional, video download does not require API keys
                  </span>
                  <div style={{ flex: 1 }} />
                  {/* Toggle */}
                  <button
              ...
              {!downloader?.exists && (
                <div className="download-instructions">
                  <div className="download-instructions-title">
                    Set up Orion Downloader
                  </div>
                  <p className="download-tool-copy">
                    Orion downloads video with <code>yt-dlp</code> and{" "}
                    <code>ffmpeg</code>. Subtitle API keys are optional and
                    only used when online subtitle search is enabled.
                  </p>
                  <div className="download-folder-row">
                    <button
                      className="btn btn-primary"
                      onClick={installTools}
                      disabled={installingTools || checking}
                    >
                      {installingTools ? "Installing..." : "Install / Repair Tools"}
                    </button>
                    <button
                      className="btn btn-secondary"
                      onClick={refreshDownloader}
                      disabled={installingTools || checking}
                    >
                      Refresh
                    </button>
                    {window.electron?.openDownloaderToolsFolder && (
                      <button
                        className="btn btn-ghost"
                        onClick={() => window.electron.openDownloaderToolsFolder()}
                      >
                        Tools Folder
                      </button>
                    )}
                    {false && downloaderFolder && (
                  ...
                  {!checking &&
                    installProgress && (
                      <div className="download-checking">
                        ...
                        {Number.isFinite(installProgress.progress)
                  ...
                  {!checking &&
                    ...
                    !downloader.exists && (
                      <div className="download-error">
                        {downloader.reason === "missing_ytdlp" &&
                          "yt-dlp is missing."}
                        {downloader.reason === "missing_ffmpeg" &&
                          "ffmpeg is missing."}
                        {(!downloader.reason ||
                          downloader.reason === "missing_tools") &&
                          "yt-dlp and ffmpeg are not ready yet."}
                        {downloader.platform !== "win32" &&
                          " Install both tools on PATH, then refresh."}
                      </div>
                    )}
                </div>
              )}
              ...
              {downloader?.exists && (
                <div className="download-ready">
                  ...
                  >
                    <span style={{ fontSize: 12, color: "var(--text3)" }}>
                      Using {downloader.ytDlp?.path || "yt-dlp"} with{" "}
                      {downloader.ffmpeg?.path || "ffmpeg"}
                    </span>
                    <button
                      className="btn btn-ghost"
