const FEATURES = [
  { id: "music", title: "Music Planet", copy: "Search, play and organize music with the living Orb, lyrics, radio, playlists and a complete player dock." },
  { id: "cinema", title: "Cinema evolved", copy: "Constellation, cast discovery, protected downloads, playback continuity and local media remain together in the story world." },
  { id: "sync", title: "One Orion backup", copy: "Your portable Cinema and Music preferences, playlists, favorites, queue and history can travel through your Google account." },
];
...
function FeatureIcon({ id }) {
  ...
  return <svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">{paths[id]}</svg>;
}

export default function WhatsNewModal({ version, onEnterMusic, onContinueCinema }) {
  return <div className="whats-new-overlay" role="dialog" aria-modal="true" aria-labelledby="whats-new-title">
    <section className="whats-new-modal">
      ...
      <div className="whats-new-features">
        {FEATURES.map((feature) => <article key={feature.id}>
